...
import com.gravirift.rifted.subrift.Utilities;
...
public class SpidermanListener implements Listener {
			...
			if (event.getAction() == Action.LEFT_CLICK_AIR) {
				if (event.getPlayer().getItemInHand().getType() == Material.STRING) {
					...
					event.getPlayer().setItemInHand(
							Utilities.reduceItemStack(event.getPlayer()
				...
				if (event.getPlayer().getItemInHand().getType() != Material.AIR) {
					if (event.getPlayer().getItemInHand().hasItemMeta()) {
						if (event.getPlayer().getItemInHand().getItemMeta()
								.getDisplayName()
								.equals(ChatColor.RESET + "Summon Spider")) {
							event.getPlayer().setItemInHand(
									Utilities.reduceItemStack(event
											.getPlayer().getItemInHand(), event
											.getPlayer()));
							Player pl = event.getPlayer();
							Spider s = (Spider) event
									.getPlayer()
									.getWorld()
									.spawnEntity(
							...
							for (Entity e : ents) {
								...
								if ((Player) e == pl)
									break;
								double eloc = e.getLocation().distanceSquared(
										pl.getLocation());
								if (eloc < dist) {
									closest = (Player) e;
									dist = eloc;
	...
	public void playerAttack(EntityDamageByEntityEvent event) {
		if (event.getDamager() instanceof Player) {
			if (KitManager.playerHasKit((Player) event.getDamager(),
					"spiderman")) {
				if (event.getEntity() instanceof LivingEntity) {
					Player pl = (Player) event.getDamager();
					if (Utilities.randInt(0, 6) == 3) {
						LivingEntity ent = (LivingEntity) event.getEntity();
						ent.addPotionEffect(new PotionEffect(
								PotionEffectType.POISON, 40, 0));
						pl.addPotionEffect(new PotionEffect(
								PotionEffectType.REGENERATION, 40, 1));
					}
