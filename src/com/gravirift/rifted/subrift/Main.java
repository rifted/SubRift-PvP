...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		getServer().getPluginManager().registerEvents(new SpidermanListener(),
				this);
		...
		if (cmd.getName().equalsIgnoreCase("kit")) {
			if (!(sender instanceof Player)) {
				sender.sendMessage(ChatColor.RED
						+ "Sorry console; you can't choose a kit!");
			} else {
				...
				if (args.length >= 1) {
					...
					if (KitManager.isKit(kit)) {
						...
						p.sendMessage(ChatColor.RED + "The kit '" + kit
								+ "' does not exist!");
					}
					return true;
				} else {
					p.sendMessage(ChatColor.YELLOW + "There are "
							+ KitManager.availableKits().size() + " kits:");
